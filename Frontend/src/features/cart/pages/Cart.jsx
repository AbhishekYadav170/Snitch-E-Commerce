...
const Cart = () => {
    ...
    useEffect(() => {
        handleGetCart().catch(() => setCartError('Could not load your cart. Please refresh the page.'))
    }, [])
    ...
    const withPending = async (key, action) => {
        ...
        try {
            ...
            setCartError(err?.response?.data?.message || 'Something went wrong. Please try again.')
        } finally {
            setPendingKeys(prev => {
                const next = { ...prev }
                delete next[ key ]
                        ...
                        <div className="w-full lg:w-[65%]">
                            ...
                            <div className="flex flex-col gap-6">
                                {cartItems.map(item => {
                                    ...
                                    const isPending = !!pendingKeys[ itemKey ]
                                            ...
                                            <div className="flex-1 flex flex-col justify-between">
                                                ...
                                                <div className="flex items-center justify-between flex-wrap gap-4">
                                                    ...
                                                    <div
                                                        ...
                                                        style={{ border: `1px solid ${tokens.outlineVariant}`, opacity: isPending ? 0.5 : 1 }}
                                                    >
                                                        <button
                                                            id={`qty-dec-${_id}`}
                                                            disabled={isPending}
                                                        ...
                                                        <button
                                                            id={`qty-inc-${_id}`}
                                                            disabled={isPending || (stock !== undefined && qty >= stock)}
                                                            onClick={() => withPending(itemKey, () => handleIncrementCartItem({ productId: _id, variantId }))}
                                                            className="w-9 h-9 flex items-center justify-center text-sm font-light transition-colors hover:opacity-60 disabled:cursor-not-allowed"
                                                    ...
                                                    <button
                                                        id={`remove-${_id}`}
                                                        disabled={isPending}
                                                        onClick={() => withPending(itemKey, () => handleRemoveCartItem({ productId: _id, variantId }))}
                                                        className="text-[10px] uppercase tracking-[0.22em] font-medium transition-all duration-200 hover:underline hover:opacity-70 disabled:cursor-not-allowed"
                                                        style={{ color: tokens.muted }}
                                                    >
                                                        {isPending ? 'Please wait...' : 'Remove'}
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
